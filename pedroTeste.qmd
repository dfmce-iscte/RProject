...
ggplot(sum_by_TRNS, aes(x = WeekDay, y = Sum_NB_VALD)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black") +
  labs(title = "Sum of NB_VALD by WeekDay", x = "WeekDay", y = "Sum of NB_VALD") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  scale_y_continuous(labels = function(x) format(x, scientific=FALSE))
```

The plot for the number of validations per day of the week shows a logical decrease when it reaches the weekend, as saturday and sunday drop to under 25M passenger validations, whereas the days of the week account for over 40M validations, except for friday, which is leading up to the weekend, showing a slight decrease in circulations.
...
ggplot(top_station_per_month, aes(x = Month, y = Total_Trips, fill = LIBELLE_ARRET)) +
...
```{r}
# Specify the weeks
week1_start <- "2017-07-01"
week1_end <- "2017-07-08"
week2_start <- "2022-07-01"
week2_end <- "2022-07-08"

# Filter data for Week 1 and print the number of validations for each day
week1_data <- final %>%
  filter(JOUR >= week1_start & JOUR <= week1_end) %>%
  mutate(WeekDay = weekdays(as.Date(JOUR))) %>%
  group_by(WeekDay) %>%
  summarise(Sum_NB_VALD = sum(NB_VALD, na.rm = TRUE))

print("Week [2017-07-01 to 2017-07-08]:")
print(week1_data)

# Filter data for Week 2 and print the number of validations for each day
week2_data <- final %>%
  filter(JOUR >= week2_start & JOUR <= week2_end) %>%
  mutate(WeekDay = weekdays(as.Date(JOUR))) %>%
  group_by(WeekDay) %>%
  summarise(Sum_NB_VALD = sum(NB_VALD, na.rm = TRUE))

print("Week [2022-07-01 to 2022-07-08]:")
print(week2_data)
```
...
APAGAR ESTE COMENTARIO teste maria